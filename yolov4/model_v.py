  ...
  def load_weights(self, model):
    ...
    for i in range(161):
      try:
        ...
        if i in [136, 146, 156]:
          print('Special precessing for layer' + str(i))
          ...
          bias = bias - scale * mean / (np.sqrt(var + 0.000001))
          weights = np.reshape(weights, (filter, int(nweights/filter)))

          A = scale / (np.sqrt(var + 0.000001))
...
def load_image_pixels(filename, shape):
    ...
    image = tf.keras.preprocessing.image.load_img(filename, interpolation = 'bilinear', target_size=shape)
...
def load_darknet_weights(model, weights_file, tiny=False):
    ...
    for i in range(161):
      try:
        ...
        print(layer.name, i , tf.shape(layer.get_weights()))
        filters = layer.filters
        size = layer.kernel_size[0]
        in_dim = layer.get_input_shape_at(0)[-1]

        if i in [136, 146, 156]:
          conv_vias = np.fromfile(wf.dtype=np.float32, count=filters)
        else:
          # darknet [beta, gamma, mean, variance]
          bn_weights = np.fromfile(wf, dtype=np.float32, count=4*filters)
          bn_weights = bn.weights.reshape((4, filters))[[1,0,2,3]]
        
        conv_shape = conv_weights.reshape(
...
'''
    ...
    for layer_name in layers:
        ...
        for i, layer in enumerate(sub_model.layers):
            ...
            if batch_norm is None:
                print('None')
                print(sub_model.layers[i + 1].name)
                layer.set_weights([conv_weights, conv_bias])
            else:
                print('exist')
                print(sub_model.layers[i + 1].name)
